...
class FiatExchangePairRateAdmin(admin.ModelAdmin):
    ...
    def conditions_time(self, obj):
        if obj.market_buy_conditions and obj.market_sell_conditions:
            return tz.localtime(obj.market_buy_conditions.created).strftime("%d/%m/%Y %H:%M")
    ...
    def buy_conditions(self, obj):
        if obj.market_buy_conditions:
            ...
            minimum = obj.market_buy_conditions.extra_data_listing.get('minSingleTransAmount', ' - ')
            
            url = reverse("admin:p2p_currencyexchangeconditions_change", args=[obj.market_buy_conditions.id])
            return format_html(f'<a href="{url}">{symbol} {price} desde {minimum}uds</a>')
    ...
    def sell_conditions(self, obj):
        if obj.market_sell_conditions:
            ...
            minimum = obj.market_sell_conditions.extra_data_listing.get('minSingleTransAmount', ' - ')
            
            url = reverse("admin:p2p_currencyexchangeconditions_change", args=[obj.market_sell_conditions.id])
            return format_html(f'<a href="{url}">{symbol} {price} desde {minimum}uds</a>')
